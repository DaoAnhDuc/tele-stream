...
import { setListKeyCamera } from "../reducer/stream/listKeyCamera";
...
class RoomClientJoinStreamClone {
    ...
    initRoomClient = async () => {
        ...
        if (create.roomname) {
            ...
            // console.log(store.getState().listLocalCameraState)
    ...
    initSocket = () => {
        ...
        this.socket.on(EVENTS.SOCKET_USER_JOIN_ROOM, async ({ name, id, isKey }) => {
            await this.getProducerOfRoom();
            Notification("info", name + " đã tham gia vào cuộc họp")

        });

        this.socket.on(EVENTS.SOCKET_USER_LEFT_ROOM, async ({ name, id, isKey }) => {
            if (isKey) {
                Notification("info", "Cuộc họp bị hủy bởi chủ phòng,màn hình sẽ đóng sau 2 giây")
        ...
        this.socket.on(RTC_EVENTS.CLOSE_CONSUME, ({ consumerId }) => {
        ...
        this.socket.on(RTC_EVENTS.NEW_PRODUCER, async (data) => {
            ...
            const listKeyCamera = { ...store.getState().listKeyCameraState };
            for (let index = 0; index < data.length; index++) {
                const { producer_id, peerName, peerId, isKey } = data[index];
                if (peerId !== this.socket.id) {
                    if (isKey) {
                        const consume = await this.consume(producer_id, peerName, peerId);
                        if (consume.kind === "audio") {
                            listKeyCamera.audio = { consume, muted: false }
                        }
                        if (consume.kind === "video") {
                            listKeyCamera.videos.push(consume)
                            listKeyCamera.videos.map((video, index) => video.active = true)
                        }
                    } else {
                        const consume = await this.consume(producer_id, peerName, peerId);
            ...
            store.dispatch(setListKeyCamera(listKeyCamera));
    ...
    createRoom = async () => {
        ...
        if (createRoom) {
            ...
            Notification("error", "Không tìm thấy phòng!")
        }
    };

    getProducerOfUserKey = async () => {
        const listProducer = await this.socket.request(RTC_EVENTS.GET_PRODUCER_OF_USER_KEY);
        const listKeyCamera = {
            audio: null,
            videos: []
        }
        for (let index = 0; index < listProducer.length; index++) {
            ...
            if (consume.kind === "audio") {
                ...
                listKeyCamera.videos.push({ id: producer_id, name: peerName, active: true, ...consume })
            }
        }
    ...
    getProducerOfRoom = async () => {
        ...
        const listKeyCamera = { audio: null, videos: [] };
        const remoteElement = [];
        for (let index = 0; index < listProducer.length; index++) {
    ...
    joinRoom = async (username, roomname, password) => {
        ...
        await this.initRTC()
        await this.getProducerOfUserKey();
        await this.getProducerOfRoom();
    };
    ...
    closeProducer = async (producer) => {
        ...
        if (producer.type === mediaType.screen) {
            ...
            await store.dispatch(setListLocalCamera(localElementState))
        }
    };
    ...
    closeConsumer = (consumerId) => {
        const remoteElement = [...store.getState().listRemoteCameraState];
        const listKeyCamera = { ...store.getState().listKeyCameraState };
        if (listKeyCamera.audio?.consume?.consumer.id === consumerId) {
            listKeyCamera.audio = null;
        } else {
            for (let index = 0; index < listKeyCamera.videos.length; index++) {
                const video = listKeyCamera.videos[index];
                if (video.consumer.id === consumerId) {
                    listKeyCamera.videos = listKeyCamera.videos.filter((video, j) => j !== index)
                    break;
                }
            }
        }
        ...
        for (let index = 0; index < remoteElement.length; index++) {
            ...
            if (element.audio?.consume.consumer.id === consumerId) {
                ...
                for (let j = 0; j < element.video.length; j++) {
                    ...
                    if (video.consumer.id === consumerId) {
                        element.video = element.video.filter((video, index) => j !== index)
                        break;
                    }
                }
            }
...
export { RoomClientJoinStreamClone }